...
def comfyui(tags):
    ...
    server_address      = '127.0.0.1:58188'
    client_id           = str(uuid.uuid4())
    
    quality_prompt      = "masterpiece, best quality,amazing quality"
    artist_prompt       = "by shinobe, by lolo \(kemonono\), by kawai_masaki, by kasugai"
    
    place_prompt        = tags
    pose_prompt         = "(aged down:1.3)" 
    character_prompt    = "silver scales, dragon tail, fluffy tail, (blonde hair, long hair, blue eyes, dragon horn, curved horn, white horn:1.05), pointy ears, (feathered wings:1.05), petite, ahoge, fangs, slit pupils, glowing eyes, dragon humanoid"
    negative_prompt     = "bad quality,worst quality,worst detail,sketch,censor"

    time_start = time.time()    
    
    print('Get tags: {}'.format(tags))
    ...
    my_gen.set_steps_cfg(steps=36, cfg=7.0, node_id="13")
    my_gen.set_seed(seed=random_integer, node_id="16")    
    my_gen.set_width_height(width=1360, height=1024, node_id="12")
    my_gen.set_postive_prompt(quality_prompt, artist_prompt, place_prompt, pose_prompt, character_prompt, "14")              
    my_gen.set_negative_prompt(negquality=negative_prompt, node_id="15")
    images = my_gen.queue_prompt()
        
    image_path = my_gen.save_images(images, current_folder+'\\..\\Output',random_integer)
    
    ws.close()            
    time_end = time.time()
    print('Time elapsed: {} min'.format((time_end - time_start)/60))
        
    #return 'http://{}/gradio_api/file=file:///{}'.format(server_address,simplify_path(image_path))
    #return 'file:///{}'.format(simplify_path(image_path))
    return simplify_path(image_path)
